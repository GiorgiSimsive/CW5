# Generated by Django 5.2.4 on 2025-08-06 08:38

import django.db.models.deletion
from django.conf import settings
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Habit',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('place', models.CharField(max_length=255)),
                ('time', models.TimeField()),
                ('action', models.CharField(max_length=255)),
                ('is_pleasant', models.BooleanField(default=False)),
                ('reward', models.CharField(blank=True, max_length=255, null=True)),
                ('periodicity', models.PositiveIntegerField(default=1, help_text='В днях')),
                ('execution_time', models.PositiveIntegerField(help_text='В секундах')),
                ('is_public', models.BooleanField(default=False)),
                ('linked_habit', models.ForeignKey(blank=True, help_text='Можно указывать только для полезных привычек', null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='linked_to', to='habits.habit')),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='habits', to=settings.AUTH_USER_MODEL)),
            ],
        ),
    ]
